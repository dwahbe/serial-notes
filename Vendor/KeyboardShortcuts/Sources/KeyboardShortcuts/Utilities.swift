...
extension Bundle {
	/**
	[Serial Notes patch — see Vendor/KeyboardShortcuts/VENDORED.md]

	SwiftPM's generated `Bundle.module` accessor only checks the `.app` root and
	the absolute `.build` path of the machine that compiled the binary, so an app
	assembled from a `swift build` binary fatalErrors on every other machine —
	code signing only allows resources under `Contents/Resources`, a location the
	accessor never searches. Prefer that location; fall back to `.module` for
	SwiftPM-native contexts (`swift test`, local tooling) where the build path
	still exists.
	*/
	static let keyboardShortcutsResources: Bundle = {
		if
			let url = Bundle.main.resourceURL?.appendingPathComponent("KeyboardShortcuts_KeyboardShortcuts.bundle"),
